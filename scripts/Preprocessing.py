...
class Preprocessing:
    def __init__(self, db_name:str, experiment_configuration, with_splits=True, with_labels_and_properties=True, data_generation=None, data_generation_args=None, create_pt_files = True):
        ...
        if type(data_generation) == str:
            ...
            if data_generation is not None:
                ...
                try:
                    ...
                    save_graphs(Path(self.experiment_configuration['paths']['data']), self.db_name, graphs, labels, with_degree=False, format='NEL')
                except:
        ...
        if with_splits:
            ...
            # generate splits
            create_splits(db_name, Path(self.experiment_configuration['paths']['data']), Path(self.experiment_configuration['paths']['splits']), folds=10, format='NEL')
    ...
    def layer_to_labels(self, layer_strings: json):
        ...
        if layer['layer_type'] == 'primary':
            save_primary_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
        elif layer['layer_type'] == 'trivial':
            save_trivial_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            ...
            if 'wl_iterations' in layer and layer['wl_iterations'] == 0:
                save_degree_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            else:
                save_wl_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], max_iterations=layer['wl_iterations'], max_label_num=layer['max_node_labels'], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            ...
            save_cycle_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], length_bound=layer['max_cycle_length'], max_node_labels=layer["max_node_labels"], cycle_type=cycle_type, label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            ...
            if 'id' in layer:
                if layer['id'] > len(self.experiment_configuration['subgraphs']):
                    ...
                    save_subgraph_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], subgraphs=subgraph_list, id=layer['id'], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            else:
            ...
            save_clique_labels(Path(self.experiment_configuration['paths']['data']), db_names=[self.db_name], max_clique=layer['max_clique_size'], max_node_labels=layer['max_node_labels'], label_path=Path(self.experiment_configuration['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
        else:
    ...
    def property_to_properties(self, property_strings: json):
        ...
        if properties['name'] == 'distances':
            ...
            write_distance_properties(Path(self.experiment_configuration['paths']['data']), db_name=self.db_name, out_path=Path(self.experiment_configuration['paths']['properties']), cutoff=properties['cutoff'], data_format='NEL', save_times=self.generation_times_properties_path)
        elif properties['name'] == 'edge_label_distances':
            if 'cutoff' not in properties:
                properties['cutoff'] = None
            write_distance_edge_properties(Path(self.experiment_configuration['paths']['data']), db_name=self.db_name, out_path=Path(self.experiment_configuration['paths']['properties']), cutoff=properties['cutoff'], data_format='NEL', save_times=self.generation_times_properties_path)
