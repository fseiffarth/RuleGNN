...
class Preprocessing:
    def __init__(self, db_name:str, config_file: Path, with_splits=True, data_generation=None, data_generation_args=None):
        ...
        if not Path(self.configs['paths']['results']).joinpath('generation_times_labels.txt').exists():
            with open(Path(self.configs['paths']['results']).joinpath('generation_times_labels.txt'), 'w') as f:
                f.write('Generation times for labels\n')
        if not Path(self.configs['paths']['results']).joinpath('generation_times_properties.txt').exists():
            with open(Path(self.configs['paths']['results']).joinpath('generation_times_properties.txt'), 'w') as f:
                f.write('Generation times for properties\n')
        self.generation_times_labels_path = Path(self.configs['paths']['results']).joinpath('generation_times_labels.txt')
        self.generation_times_properties_path = Path(self.configs['paths']['results']).joinpath('generation_times_properties.txt')

        if type(data_generation) == str:
            if data_generation == 'TUDataset':
                try:
                    tu_to_nel(db_name=db_name, out_path=Path(self.configs['paths']['data']))
                except:
    ...
    def layer_to_labels(self, layer_strings: json):
        ...
        if layer['layer_type'] == 'primary':
            save_primary_labels(Path(self.configs['paths']['data']), db_names=[self.db_name], label_path=Path(self.configs['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
        elif layer['layer_type'] == 'trivial':
            save_trivial_labels(Path(self.configs['paths']['data']), db_names=[self.db_name], label_path=Path(self.configs['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            ...
            if 'wl_iterations' in layer and layer['wl_iterations'] == 0:
                save_degree_labels(Path(self.configs['paths']['data']), db_names=[self.db_name], label_path=Path(self.configs['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            else:
                save_wl_labels(Path(self.configs['paths']['data']), db_names=[self.db_name], max_iterations=layer['wl_iterations'], max_label_num=layer['max_node_labels'], label_path=Path(self.configs['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
            ...
            save_cycle_labels(Path(self.configs['paths']['data']), db_names=[self.db_name], length_bound=layer['max_cycle_length'], max_node_labels=layer["max_node_labels"], cycle_type=cycle_type, label_path=Path(self.configs['paths']['labels']), format='NEL', save_times=self.generation_times_labels_path)
        elif layer['layer_type'] == 'subgraph':
            if 'id' in layer:
                if layer['id'] > len(self.configs['subgraphs']):
                    ...
                    subgraph_list = eval(self.configs['subgraphs'][layer['id']])
    ...
    def property_to_properties(self, property_strings: json):
        ...
        if properties['name'] == 'distances':
            ...
            write_distance_properties(Path(self.configs['paths']['data']), db_name=self.db_name, out_path=Path(self.configs['paths']['properties']), cutoff=properties['cutoff'], data_format='NEL', save_times=self.generation_times_properties_path)
        elif properties['name'] == 'edge_label_distances':
            if 'cutoff' not in properties:
                properties['cutoff'] = None
            write_distance_edge_properties(Path(self.configs['paths']['data']), db_name=self.db_name, out_path=Path(self.configs['paths']['properties']), cutoff=properties['cutoff'], data_format='NEL', save_times=self.generation_times_properties_path)
