...
def main():
    ...
    channel = 0
    sort_indices = rules_vs_occurences(convolution_layer, channel)
    ...
    for run_id in range(3):
        ...
        convolution_layer.draw(ax=axs[run_id][1], graph_id=graph_id, graph_drawing=graph_drawing, filter_weights=None)
        convolution_layer.draw(ax=axs[run_id][2], graph_id=graph_id, graph_drawing=graph_drawing, filter_weights={'absolute': 10})
        convolution_layer.draw(ax=axs[run_id][3], graph_id=graph_id, graph_drawing=graph_drawing, filter_weights={'absolute': 3})
        aggregation_layer.draw(ax=axs[run_id][4], graph_id=graph_id, graph_drawing=graph_drawing, out_dimension=0)
        aggregation_layer.draw(ax=axs[run_id][5], graph_id=graph_id, graph_drawing=graph_drawing, out_dimension=1)
    plt.show()
    return
