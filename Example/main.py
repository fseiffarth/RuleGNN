...
def create_labels(data_path: Path):
...
def create_properties(data_path: Path):
    write_distance_properties(data_path, db_name='EXAMPLE_DB', out_path=data_path.joinpath('Properties/'), cutoff=None, data_format='NEL')
    write_distance_edge_properties(data_path, db_name='EXAMPLE_DB', out_path=data_path.joinpath('Properties/'), cutoff=1, data_format='NEL')



def main(save_data=True, with_splits=True, with_labels=True, with_properties=True):
    data_path = Path('Example/Data/')
    if save_data:
    ...
    if with_splits:
        ...
        create_splits(db_name='EXAMPLE_DB', path=data_path, output_path=data_path.joinpath('Splits/'), format='NEL')
    if with_labels:
        # create labels
