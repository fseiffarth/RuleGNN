...
def get_existing_splits():
    ...
    Path("Examples/TUExample/Data").mkdir(exist_ok=True)
    Path("Examples/TUExample/Data/Splits").mkdir(exist_ok=True)
    # copy the splits for NCI1, IMDB-BINARY, IMDB-MULTI and CSL
    for split in ["NCI1", "IMDB-BINARY", "IMDB-MULTI", "CSL"]:
        Path.write_text(Path("Examples/TUExample/Data/Splits").joinpath(f"{split}_splits.json"), Path("Data/Splits").joinpath(f"{split}_splits.json").read_text())
