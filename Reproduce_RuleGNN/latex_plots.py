    ...
    if not Path(f'Reproduce_RuleGNN/Results/Latex/Plots/visualization_{db_name}_{graph_id_string}.pdf').exists():
        ...
        if len(graph_ids) == 1:
            ...
            for i in range(0, num_convolution_layers):
                for j, filter_weights in enumerate(filtering):
                    if filter_weights is None:
                        axs[1+column_for_invariants+ i*len(filtering) + j].set_title(f'Learned Parameters')
                    elif 'absolute' in filter_weights:
                        axs[1+column_for_invariants+ i*len(filtering) + j].set_title(f'Top ${filter_weights["absolute"]}$ Parameters')
            ...
            for i in range(0, num_convolution_layers):
                for j, filter_weights in enumerate(filtering):
                    if filter_weights is None:
                        axs[0][1 + column_for_invariants + i*len(filtering) + j].set_title(f'Learned Parameters')
                    elif 'absolute' in filter_weights:
                        axs[0][1 + column_for_invariants + i*len(filtering) + j].set_title(f'Top ${filter_weights["absolute"]}$ Parameters')
...
def rules_vs_weights(layer:RuleConvolutionLayer, sort_indices:np.ndarray, steps,db_name, channel=0):
    if not Path(f'Reproduce_RuleGNN/Results/Latex/Plots/weights_per_rule_{db_name}.png').exists():
...
def plot_shared_weights(path, db_name):
    paths = [f'Reproduce_RuleGNN/Results/Latex/Plots/occurrences_per_rule_{db_name}.png',
