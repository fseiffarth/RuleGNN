...
from Reproduce_RuleGNN.latex import share_gnn_results
...
def ablation_threshold(dataset, threshold_type):
    if not Path(f'Reproduce_RuleGNN/Results/Latex/Plots/ablation_threshold_{dataset}_{threshold_type}.pdf').exists():
        plt.rcParams.update({
        ...
        ablation_results_path = Path(f'Reproduce_RuleGNN/Results/Ablation/{threshold_type}/')
...
def plot_network(path, db_name, graph_ids, filtering, draw_type=None, with_labels_from_invariant=True, with_aggregation=False, molecule=False, channel=0):
    ...
    if not Path(f'Reproduce_RuleGNN/Results/Latex/Plots/visualization_{db_name}_{graph_id_string}.pdf').exists():
        #mpl.use("pgf")
        import matplotlib.pyplot as plt
        ...
        Path('Reproduce_RuleGNN/Results/Latex/Plots/Positions/').mkdir(exist_ok=True, parents=True)
        save_pos_path = Path('Reproduce_RuleGNN/Results/Latex/Plots/Positions/')
...
def rules_vs_weights(layer:RuleConvolutionLayer, sort_indices:np.ndarray, steps,db_name, channel=0, appendix=''):
    if not Path(f'Reproduce_RuleGNN/Results/Latex/Plots/weights_per_rule_{db_name}{appendix}.png').exists():
...
def plot_shared_weights(path, db_name, appendix=''):
    ...
    paths = [f'Reproduce_RuleGNN/Results/Latex/Plots/occurrences_per_rule_{db_name}{appendix}.png',
             f'Reproduce_RuleGNN/Results/Latex/Plots/weights_per_rule_{db_name}{appendix}.png']
...
def main():
    ...
    Path('Reproduce_RuleGNN/Results/Latex/Plots').mkdir(parents=True, exist_ok=True)
    plot_network_path = 'Reproduce_RuleGNN/Configs/main_config_fair_real_world.yml'
    ...
    plot_network_path_ablation_threshold = lambda x : f'Reproduce_RuleGNN/Configs/ablation/threshold/lower/main_config_ablation_threshold_{x}.yml'
    plot_network_path_ablation_distance = 'Reproduce_RuleGNN/Configs/ablation/distances/main_config_ablation_distances.yml'
