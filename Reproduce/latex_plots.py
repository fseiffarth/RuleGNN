...
def ablation_threshold(dataset, threshold_type):
    if not Path(f'Reproduce/Results/Latex/Plots/ablation_threshold_{dataset}_{threshold_type}.pdf').exists():
        ...
        ablation_results_path = Path(f'Reproduce/Results/Ablation/{threshold_type}/')
        ...
        if threshold_type == 'Lower':
            ...
            ax1.set_xlabel('Minimum \\# of Occurrences per Shared Weight (Encoder)')
        plt.savefig(f'Reproduce/Results/Latex/Plots/ablation_threshold_{dataset}_{threshold_type}.pdf', bbox_inches='tight', backend='pgf')
        pass

def ablation_distance(dataset='NCI1', max_distance=12, fontsize=9):
    if not Path(f'Reproduce/Results/Latex/Plots/ablation_distance_{dataset}_{max_distance}.pdf').exists():
        plt.rcParams.update({
            "font.family": "serif",  # use serif/main font for text elements
            "font.size": 12,
            "text.usetex": True,  # use inline math for ticks
        ...
        path = Path(f'Reproduce/Results/Distance/{dataset}/')
        ...
        plt.savefig(f'Reproduce/Results/Latex/Plots/ablation_distance_{dataset}_{max_distance}.pdf', bbox_inches='tight', backend='pgf')
        pass
...
def rules_vs_occurences(layer: RuleConvolutionLayer, db_name, channel=0, appendix='') -> np.ndarray:
    if not Path(f'Reproduce/Results/Latex/Plots/occurrences_per_rule_{db_name}{appendix}.png').exists():
        ...
        plt.savefig(f'Reproduce/Results/Latex/Plots/occurrences_per_rule_{db_name}{appendix}.png', bbox_inches='tight')
...
def rules_vs_weights(layer:RuleConvolutionLayer, sort_indices:np.ndarray, steps,db_name, channel=0, appendix=''):
    if not Path(f'Reproduce/Results/Latex/Plots/weights_per_rule_{db_name}{appendix}.png').exists():
        ...
        plt.savefig(f'Reproduce/Results/Latex/Plots/weights_per_rule_{db_name}{appendix}.png', bbox_inches='tight')


def plot_shared_weights(path, db_name, appendix=''):
    if appendix != '':
        appendix = f'_{appendix}'
    paths = [f'Reproduce/Results/Latex/Plots/occurrences_per_rule_{db_name}{appendix}.png',
             f'Reproduce/Results/Latex/Plots/weights_per_rule_{db_name}{appendix}.png']
...
def main():
    ...
    Path('Reproduce/Results/Latex/Plots').mkdir(parents=True, exist_ok=True)
    plot_network_path = 'Reproduce/Configs/main_config_fair_real_world.yml'
