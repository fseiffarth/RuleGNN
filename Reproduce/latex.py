...
def fair_table():
    ...
    for i, algorithm in enumerate(baseline_algorithms):
        row_string, row_results = baseline_results(algorithm, datasets, 'Reproduce/Results/RealWorld/Baseline/', first_column=baseline_first_columns[i])
    ...
    row, row_results = share_gnn_results('\\MyGNN (ours)', datasets, 'Reproduce/Results/RealWorld/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Random (ours)', datasets, 'Reproduce/Results/RealWorld/Random/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Encoder (ours)', datasets, 'Reproduce/Results/RealWorld/Encoder/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Decoder (ours)', datasets, 'Reproduce/Results/RealWorld/Decoder/')
    rows.append(row)
    results.append(row_results)
...
def fair_table_full():
    ...
    synthetic_results = []
    for i, algorithm in enumerate(baseline_algorithms):
        row_string, row_results = baseline_results(algorithm, synthetic_datasets, 'Reproduce/Results/Synthetic/Baseline/', first_column=baseline_first_columns[i])
    ...
    row, row_results = share_gnn_results('\\MyGNN (ours)', synthetic_datasets, 'Reproduce/Results/Synthetic/')
    synthetic_rows.append(row)
    synthetic_results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Random (ours)', synthetic_datasets, 'Reproduce/Results/Synthetic/Random/')
    synthetic_rows.append(row)
    synthetic_results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Encoder (ours)', synthetic_datasets, 'Reproduce/Results/Synthetic/Encoder/')
    synthetic_rows.append(row)
    synthetic_results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Decoder (ours)', synthetic_datasets, 'Reproduce/Results/Synthetic/Decoder/')
    synthetic_rows.append(row)
    synthetic_results.append(row_results)
    # results to numpy array
...
def sota_baseline_and_share():
    ...
    baseline_first_columns = ['\\cite{Schulz2019OnTN}', '\\cite{DBLP:journals/jmlr/ShervashidzeSLMB11}']
    for i, algorithm in enumerate(baseline_algorithms):
        row_string, row_results = baseline_results(algorithm, datasets, 'Reproduce/Results/Sota/Baseline/', first_column=baseline_first_columns[i], sota=True)
        rows.append(row_string)
        results.append(row_results)

    row, row_results = share_gnn_results('\\MyGNN (ours)', datasets, 'Reproduce/Results/Sota/', sota=True)
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Random (ours)', datasets, 'Reproduce/Results/Sota/Random/', sota=True)
    rows.append(row)
    results.append(row_results)
    first_columns = ([f'{x} {baseline_first_columns[i]}' for i, x in enumerate(baseline_algorithms)]
...
def synthetic_table():
    ...
    baseline_first_columns = ['\\cite{Schulz2019OnTN}', '\\cite{DBLP:journals/jmlr/ShervashidzeSLMB11}']
    for i, algorithm in enumerate(baseline_algorithms):
        row_string, row_results = baseline_results(algorithm, datasets, 'Reproduce/Results/Synthetic/Baseline/', first_column=baseline_first_columns[i])
        rows.append(row_string)
        results.append(row_results)
    ...
    row, row_results = share_gnn_results('\\MyGNN', datasets, 'Reproduce/Results/Synthetic/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Random', datasets, 'Reproduce/Results/Synthetic/Random/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Encoder', datasets, 'Reproduce/Results/Synthetic/Encoder/')
    rows.append(row)
    results.append(row_results)
    row, row_results = share_gnn_results('\\MyGNN-Decoder', datasets, 'Reproduce/Results/Synthetic/Decoder/')
    rows.append(row)
    results.append(row_results)
...
def training_and_preprocessing_time(share_gnn_type=''):
    ...
    if Path(f'Reproduce/Results/Latex/training_preprocessing_time{appendix}.json').exists():
        results = json.load(open(f'Reproduce/Results/Latex/training_preprocessing_time{appendix}.json', 'r'))
    else:

        path_real_world = f'Reproduce/Results/RealWorld/{share_gnn_type}'
        path_synthetic = f'Reproduce/Results/Synthetic/{share_gnn_type}'
        for path, datasets in [[path_real_world, datasets_real_world], [path_synthetic, dataset_synthetic]]:
            if Path(path).exists():
                ...
                if share_gnn_type == '':
                    ...
                    config_path = Path('Reproduce/Configs/main_config_fair_real_world_random_variation.yml')
                else:
                    raise ValueError('share_gnn_type not recognized')
                if path == path_synthetic:
                    if share_gnn_type == '':
                        config_path = Path('Reproduce/Configs/main_config_fair_synthetic.yml')
                    elif share_gnn_type == 'Random':
                        config_path = Path('Reproduce/Configs/main_config_fair_synthetic_random_variation.yml')
                    else:
                        raise ValueError('share_gnn_type not recognized')
        ...
        with open(f'Reproduce/Results/Latex/training_preprocessing_time{appendix}.json', 'w') as f:
            json.dump(results, f)
    ...
    with open(f'Reproduce/Results/Latex/best_run_details_table{appendix}.txt', 'w') as f:
        f.write(table_str)
    ...
    if share_gnn_type == '':
        ...
        table_str += '\\end{tabular}\n'
        # save table under best run properties table
        with open('Reproduce/Results/Latex/preprocessing_times.txt', 'w') as f:
            f.write(table_str)
...
def hyper_parameter_configurations():
    ...
    json_files = ['Reproduce/Results/Latex/molecule_convolution_configs.json',
                    'Reproduce/Results/Latex/molecule_aggregation_configs.json',
                    'Reproduce/Results/Latex/social_convolution_configs.json',
                    'Reproduce/Results/Latex/social_aggregation_configs.json']
    if all([Path(x).exists() for x in json_files]):
        ...
        for run_configs in [run_configs_molecule, run_configs_social]:
            ...
            if run_configs == run_configs_molecule:
                with open(f'Reproduce/Results/Latex/molecule_convolution_configs.json', 'w') as f:
                    json.dump(list(convolution_configs), f)
                with open(f'Reproduce/Results/Latex/molecule_aggregation_configs.json', 'w') as f:
                    json.dump(list(aggregation_configs), f)
            elif run_configs == run_configs_social:
                with open(f'Reproduce/Results/Latex/social_convolution_configs.json', 'w') as f:
                    json.dump(list(convolution_configs), f)
                with open(f'Reproduce/Results/Latex/social_aggregation_configs.json', 'w') as f:
                    json.dump(list(aggregation_configs), f)
    molecule_convolution_configs = json.load(
        open('Reproduce/Results/Latex/molecule_convolution_configs.json', 'r'))
    molecule_aggregation_configs = json.load(
        open('Reproduce/Results/Latex/molecule_aggregation_configs.json', 'r'))
    social_convolution_configs = json.load(open('Reproduce/Results/Latex/social_convolution_configs.json', 'r'))
    social_aggregation_configs = json.load(open('Reproduce/Results/Latex/social_aggregation_configs.json', 'r'))
    ...
    for x, y, z in zip([molecule_convolution_configs, molecule_aggregation_configs, social_convolution_configs, social_aggregation_configs],
        ...
        with open(f'Reproduce/Results/Latex/{z}.txt', 'w') as f:
            f.write(table_string)

def main():
    # create Latex dir under Results
    Path('Reproduce/Results/Latex').mkdir(parents=True, exist_ok=True)
