...
from Reproduce import experiments_fair_real_world, experiments_standard_real_world, experiments_synthetic, \
    experiments_ablation_distance, experiments_ablation_threshold, get_gnn_comparison_data, latex, \
    latex_plots
...
def get_existing_splits():
    ...
    # copy all split files from "Data/Splits" to "Reproduce/Data/Splits"
    for file in Path("Data/Splits").glob("*.json"):
        target_path = Path("Reproduce/Data/Splits").joinpath(file.name)
        target_path.write_text(file.read_text())

    # copy all split files from "Data/SplitsSimple" to "Reproduce/Data/SplitsSimple"
    Path("Reproduce/Data/SplitsSimple").mkdir(exist_ok=True)
    for file in Path("Data/SplitsSimple").glob("*.json"):
        target_path = Path("Reproduce/Data/SplitsSimple").joinpath(file.name)
        target_path.write_text(file.read_text())
