...
def main():
    for db_name in ['IMDB-BINARY', 'IMDB-MULTI']:
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_real_world_random_variation.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
                                db_name=db_name)
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_real_world_random_variation.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
                                db_name=db_name, with_features=False)
    for db_name in ['DHFR', 'NCI1', 'NCI109', 'Mutagenicity']:
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_real_world.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
                                db_name=db_name)
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_real_world.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
    ...
    for db_name in ['CSL', 'EvenOddRings2_16', 'EvenOddRingsCount16', 'LongRings100', 'Snowflakes']:
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_synthetic.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
                                db_name=db_name)
        get_gnn_comparison_data(main_config_path=Path('Reproduce/Configs/main_config_fair_synthetic.yml'),
                                output_path=Path(f'Reproduce/DataGNNComparison/'),
