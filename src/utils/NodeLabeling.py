...
def standard_node_labeling(graphs: List[nx.Graph]):
    ...
    :param graph_data: BenchmarkGraphs object
...
def weisfeiler_lehman_node_labeling(graphs: List[nx.Graph], depth: int = 3, labeled: bool = False):
    unique_node_labels = []
    db_unique_node_labels = {}
    union_graph = nx.disjoint_union_all(graphs)
    hash_dict = []
    if labeled:
        hashes = nx.weisfeiler_lehman_subgraph_hashes(union_graph, iterations=depth, node_attr='label')
    else:
...
def weisfeiler_lehman_node_labeling(graphs: List[nx.Graph], depth: int = 3, labeled: bool = False):
    ...
    hash_dict = []
    if labeled:
        # if node data[label] is a list take only the first element and declare as node_attr='primary_label'
        for node in union_graph.nodes(data=True):
            if 'label' in node[1]:
                if type(node[1]['label']) == int or type(node[1]['label']) == float:
                    node[1]['primary_label'] = node[1]['label']
                elif len(node[1]['label']) > 0:
                    node[1]['primary_label'] = node[1]['label'][0]
                else:
                    node[1]['primary_label'] = 0
