...
class NoGKernelWLNN():
    def __init__(self, run: int, k_val: int, graph_data: GraphData, training_data: List[int], validate_data: List[int],
                 test_data: List[int], seed: int, para: Parameters, results_path: str):
        self.run = run
        self.k_val = k_val
    ...
    def Run(self):
        ...
        num_unique_node_labels = self.graph_data.primary_node_labels.num_unique_node_labels + self.graph_data.secondary_node_labels.num_unique_node_labels
        ...
        for i in range(0, self.graph_data.num_graphs):
            for j in range(0, num_unique_node_labels):
                if j < self.graph_data.primary_node_labels.num_unique_node_labels:
                    ...
                    k = j - self.graph_data.primary_node_labels.num_unique_node_labels
                    if k in self.graph_data.secondary_node_labels.unique_node_labels[i]:
                        X[i, j] = self.graph_data.secondary_node_labels.unique_node_labels[i][k]
    ...
    class NeuralNet(torch.nn.Module):
        def __init__(self, n_input, n_hidden, n_output):
            super(NoGKernelWLNN.NeuralNet, self).__init__()
